...
def create_scheduler_tab() -> gr.Tab:
    """Create the scheduler management tab."""

    with gr.Tab("⏰ 定时任务") as tab:
        # Header
        gr.Markdown("""
        ### ⏰ 定时任务管理
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                with gr.Group(elem_classes=["gr-box"]):
                    ...
                    available_metrics = (
                        registry.list_by_category(__import__('src.models.metric_result', fromlist=['MetricCategory']).MetricCategory.RETRIEVAL) +
                        registry.list_by_category(__import__('src.models.metric_result', fromlist=['MetricCategory']).MetricCategory.GENERATION) +
                        registry.list_by_category(__import__('src.models.metric_result', fromlist=['MetricCategory']).MetricCategory.FAQ) +
                        registry.list_by_category(__import__('src.models.metric_result', fromlist=['MetricCategory']).MetricCategory.COMPREHENSIVE)
                    )
        ...
        # Refresh task list
        refresh_btn.click(
            fn=lambda: run_async(load_scheduled_tasks()),
            outputs=[task_list],
        )

        # Scheduler controls
        start_scheduler_btn.click(
            fn=lambda r: run_async(toggle_scheduler(r)),
            inputs=[scheduler_running],
            outputs=[scheduler_status, scheduler_running],
        )

        stop_scheduler_btn.click(
            fn=lambda r: run_async(toggle_scheduler(not r)),
            inputs=[scheduler_running],
            outputs=[scheduler_status, scheduler_running],
        )
        ...
        # Save task
        save_task_btn.click(
            fn=lambda *args: run_async(save_scheduled_task(*args)),
            inputs=[
                task_name,
        ...
        delete_task_btn.click(
            fn=lambda evt: run_async(delete_selected_task(evt)),
            outputs=[task_status],
        )

        # Load history
        refresh_history_btn.click(
            fn=lambda f: run_async(load_execution_history(f)),
        ...
        # Quick actions
        run_now_btn.click(
            fn=lambda evt: run_async(run_task_now(evt)),
            outputs=[action_status],
        )

        clear_completed_btn.click(
            fn=lambda: run_async(clear_completed_tasks()),
            outputs=[action_status],
        )
        ...
        tab.select(
            fn=lambda: (
                run_async(load_scheduled_tasks()),
                run_async(load_execution_history("全部")),
            ),
            outputs=[task_list, history_table],
        )
