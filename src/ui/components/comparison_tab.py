...
def create_comparison_tab() -> None:
    ...
    async def load_comparison_data(run_id: str):
        ...
        iface1_stats_data = run.summary_by_interface.get(iface1_name, {})
        iface2_stats_data = run.summary_by_interface.get(iface2_name, {})

        # Build comparison summary
        avg1 = iface1_stats_data.get('average_score', 0)
        avg2 = iface2_stats_data.get('average_score', 0)
        diff = avg1 - avg2
        ...
        success_rate1 = iface1_stats_data.get('success_rate', 0)
        success_rate2 = iface2_stats_data.get('success_rate', 0)
        ...
        **结论**: {winner}
        ...
        if results_1 and results_2:
            ...
            for name in sorted(all_metric_names):
                avg1_m = sum(metrics_1.get(name, [0])) / len(metrics_1.get(name, [1])) if metrics_1.get(name) else 0
                avg2_m = sum(metrics_2.get(name, [0])) / len(metrics_2.get(name, [1])) if metrics_2.get(name) else 0
                diff_m = avg1_m - avg2_m
                ...
                metric_rows.append([name, round(avg1_m, 4), round(avg2_m, 4), round(diff_m, 4), winner_icon])
        ...
        return (
            ...
            gr.update(value=iface1_stats_data),
            gr.update(value=iface2_stats_data),
            comparison_md,
...
"""
    ...
    async def prev_result(idx, r1, r2):
        """上一个结果"""
        return await navigate_result(-1, idx, r1, r2)

    async def next_result(idx, r1, r2):
        """下一个结果"""
        return await navigate_result(1, idx, r1, r2)
